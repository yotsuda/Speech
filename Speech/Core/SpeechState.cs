...
namespace Speech.Core
{
    ...
    /// 音声出力のグローバル状態管理
    /// </summary>
    public static class SpeechState
    {
        private static readonly ConcurrentQueue<WindowsSpeechRequest> _outputQueue = new();
        private static CancellationTokenSource? _currentCancellationTokenSource;
        private static int _isProcessing = 0;

        public static void EnqueueOutput(WindowsSpeechRequest request)
        {
